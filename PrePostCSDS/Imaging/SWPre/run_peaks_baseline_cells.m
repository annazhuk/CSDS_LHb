...
for mousenum = 5:size(mice,1)
    Fdata_file = 'm555_SW_Pre_baseline.sch.csv';
...
filename = 'celldata_555_SWPre_baseline.mat';
for m = 1:size(mice,1)
...
% res
Pre_res_im = res_im;
for m = 1:size(Pre_res_im.mice,1)
    Pre_res_im.peak_totals_mouse(m,1) = nanmean(Pre_res_im.pk_totals{m});
end

% sus
Pre_sus_im = sus_im;
for m = 1:size(Pre_sus_im.mice,1)
    Pre_sus_im.peak_totals_mouse(m,1) = nanmean(Pre_sus_im.pk_totals{m});
end

%% save
sus_cell_total_pre = sus_cell_total;
res_cell_total_pre = res_cell_total;
save('baseline_peaks_PRE_res_sus_NEW.mat','Pre_sus_im','Pre_res_im','sus_cell_total_pre','res_cell_total_pre','cropped_SI_sorted_im','cropped_ID_sorted_im')