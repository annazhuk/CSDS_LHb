function SI_cells_nose_imaging_res_sus_NEW(AKRPre,SW_CSDS_Post,mouse_CSDS,SW_CTRL_Post,CSDS_ID,CSDS_SI)
...
% res
r = 1;
for i = 1:size(AKRPre.mice,1)
    if ismember(AKRPre.mice(i),resID)
        res_im.mice(r,1) = AKRPre.mice(i);
        res_im.nose_plot_start{r,1} = AKRPre.nose_plot_start{i};
        res_im.nose_plot_stop{r,1} = AKRPre.nose_plot_stop{i};
        res_im.nose_plot_start_avg{r,1} = AKRPre.nose_plot_start_avg{i};
        res_im.nose_plot_stop_avg{r,1} = AKRPre.nose_plot_stop_avg{i};
        res_im.nose_plot_start_avg_FP(r,:) = AKRPre.nose_plot_start_avg_FP(i,:);
        res_im.nose_plot_stop_avg_FP(r,:) = AKRPre.nose_plot_stop_avg_FP(i,:);
        r = r + 1;
    end
end
...
s = 1;
for i = 1:size(AKRPre.mice,1)
    if ismember(AKRPre.mice(i),susID)
        sus_im.mice(s,1) = AKRPre.mice(i);
        sus_im.nose_plot_start{s,1} = AKRPre.nose_plot_start{i};
        sus_im.nose_plot_stop{s,1} = AKRPre.nose_plot_stop{i};
        sus_im.nose_plot_start_avg{s,1} = AKRPre.nose_plot_start_avg{i};
        sus_im.nose_plot_stop_avg{s,1} = AKRPre.nose_plot_stop_avg{i};
        sus_im.nose_plot_start_avg_FP(s,:)  = AKRPre.nose_plot_start_avg_FP(i,:);
        sus_im.nose_plot_stop_avg_FP(s,:)  = AKRPre.nose_plot_stop_avg_FP(i,:);
        s = s + 1;
    end
end
...
% cell bodies
cropped_ID_im = CSDS_ID(find(ismember(CSDS_ID,AKRPre.mice)));
cropped_SI_im = CSDS_SI(find(ismember(CSDS_ID,AKRPre.mice)));
% make sure croppedSIs are in the same order as struct with Favg data
[~,idx_im] = sort(AKRPre.mice);
...
%% save
save('SI_cells_imaging_visits_nose_res_sus_NEW.mat','sus_im','res_im','AKRPre','cropped_ID_im_sorted','cropped_SI_im_sorted')