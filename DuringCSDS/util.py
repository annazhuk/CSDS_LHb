...
def make_behavior_plots(dat,height=1,aspect=3,offset=0.2,titles='{row_name}',ylabel='',save_names='SI_vs',
    ...
    g=sns.relplot(data=dat,x='d',y='t',hue='rb',col='c',kind='line',legend=False,
               height=height, markers=True,aspect=1/aspect,ci=68, palette=pal2,col_order=order,
                  err_style=err_style,
    ...
    if len(save_names):
        plt.savefig(save_names+'_over_time.pdf',dpi=300,transparent=True,format='pdf')
    plt.show()
    
    FSIZE=g.fig.get_size_inches()
    #FSIZE=(4,2.5) #Anna: added
    ...
    for ax,rowtitle,title in zip(axs,order,titles):
        ...
        if 'sex' in meandictdf.columns:
            sns.scatterplot(data=meandictdf.query('sex=="male"'),y='t',x='si',hue='rb',#err_types='bars'
                          legend=False,s=50,ci=68, palette=pal2,ax=ax,alpha=0.8,zorder=500,marker='o')
            sns.scatterplot(data=meandictdf.query('sex=="male"'),y='t',x='si',hue='rb',#err_types='bars'
                          legend=False,s=50,ci=68, palette=pal2,ax=ax,alpha=0.8,zorder=500,marker='o')
            sns.scatterplot(data=meandictdf.query('sex=="female"'),y='t',x='si',hue='rb',#err_types='bars'
                          legend=False,s=50,ci=68, palette=pal2,ax=ax,alpha=0.8,zorder=500,marker='^')
            sns.scatterplot(data=meandictdf.query('sex=="female"'),y='t',x='si',hue='rb',#err_types='bars'
                          legend=False,s=50,ci=68, palette=pal2,ax=ax,alpha=0.8,zorder=500,marker='^')
        
        else:
            sns.scatterplot(data=meandictdf,y='t',x='si',hue='rb',markers=True,#err_types='bars'
                      legend=False,s=20,ci=68, palette=pal2,ax=ax,alpha=0.8,zorder=500,marker='o')
    ...
    plt.tight_layout()
    plt.gcf().subplots_adjust(left=0.23)
    #plt.figure(figsize=(4,2.5)) #Anna: added
    if len(save_names):
        plt.savefig(save_names+'_scatter_six.pdf',dpi=300,transparent=True,format='pdf',bbox_inches='tight')
    plt.show()
